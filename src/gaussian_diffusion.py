...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.normalize_nll_loss = normalize_nll_loss
    ...
    def p_mean_variance(
        ...
        model_output = model.forward_with_cfg(x, self._scale_timesteps(t), cond, **model_kwargs)
    ...
    def p_sample_loop(
        ...
        """
        final = []
        loop_fn = self.p_sample_loop_progressive if langevin_fn is not None else self.p_sample_loop_langevin_progressive

        for sample in loop_fn(
            model,
            shape,
            cond=cond,
            noise=noise,
        ...
        for i in indices: # from T to 0
            ...
            with th.no_grad():
                ...
                if langevin_fn is not None:
                    out["sample"] = langevin_fn(x_t = out["sample"], t = t, labels = cond)
                yield out
                sample_x = out["sample"]
    ...
    def _token_discrete_loss(self, x_t, get_logits, input_ids):
        '''
    ...
    def training_losses_e2e(self, model, input_ids_x, t, cond = None, mask = None, model_kwargs=None, noise=None):
        """
        ...
        x_start_mean = model.model.get_embeds(input_ids_x, mask)
        ...
        model_output = model(x_t, self._scale_timesteps(t), cond, mask)
        ...
        # terms["nll"] = self._to÷ken_discrete_loss(model_out_x_start, get_logits, input_ids_x, truncate=True, t=t) # x_0->model_out_x_start
        # assert (model.lm_head.weight == model.word_embedding.weight).all()
        terms["decoder_nll"] = decoder_nll

        if self.normalize_nll_loss:
            terms["loss"] = terms["mse"] + (decoder_nll + tT_loss) * (100./(self.num_timesteps - 1))
        else:
            terms["loss"] = terms["mse"] + decoder_nll + tT_loss 
        # terms["loss"] = terms["mse"] + terms["tT_loss"]
...
class _WrappedModel:
    ...
    def __call__(self, x, ts, cond=None, mask=None, **kwargs):
        # print(ts)
        ...
        # return temp
        # print(new_ts)
        return self.model(x, new_ts, cond, mask, **kwargs)

    def __getattr__(self, name):
        """
        Forward any attribute or method that isn't found in _WrappedModel
        to the original model. This ensures that we can call any method of the
        original model, including `forward_with_cfg`.
        """
